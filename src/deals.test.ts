...
describe("getCurrentDeals", () => {
  ...
  it("shows alcohol only when its category is selected", () => {
    expect(
      getCurrentDeals(deals, {
        category: "Alcohol",
        supermarket: "All",
    ...
    ).toEqual(["current-alcohol"]);
  });
});

describe("getSupermarkets", () => {
  it("returns unique supermarkets in display order", () => {
    expect(getSupermarkets(deals)).toEqual(["Krolow", "Mercado Prado", "Roxo"]);
  });
});
